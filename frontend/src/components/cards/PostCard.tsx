        ...
        <Link to={"/~/" + postdetails.communityname + "/comments/" + postdetails.postid} className="flex flex-col w-[360px] md:w-[600px] cursor-pointer hover:bg-[#1f211d] rounded-xl mb-3 p-2 useinter">
          <div className="flex w-full justify-between">
            ...
            <div className="flex w-[1/4] relative justify-center items-center mb-2">
              <button className="ps-4 pe-4 bg-[#A9A74F] hover:bg-[#A9A74F]/75 rounded-full text-xs h-8 justify-center items-center" onClick={(e) => e.preventDefault()}>Join</button>
              <button ref={postMenuRef} className='font-bold ms-2 flex justify-center items-center h-10 w-10 bg-transparent rounded-full useinter text-sm hover:bg-[#30312f]' onClick={(e) => {e.preventDefault(); setPostMenu(!postMenu)}}><DotsThree size={16}/></button>
              <AnimatePresence>
                {postMenu && (
          ...
          <div className="flex w-full">
            ...
            <div className="flex p-2 ps-3 pe-3 rounded-full bg-[#1a1b18] me-2 cursor-pointer hover:bg-[#383c34]" onClick={(e) => e.preventDefault()}>
              <ChatCenteredText size={20} className="me-1"/>
              <span className="text-sm">{postdetails.commentcount}</span>
            </div>
            <div className="flex p-2 ps-3 pe-3 rounded-full bg-[#1a1b18] cursor-pointer hover:bg-[#383c34]" onClick={(e) => e.preventDefault()}>
            ...
            <Link to={"/~/" + postdetails.communityname + "/comments/" + postdetails.postid} className="flex w-full mb-2 me-4 rounded-xl">
              ...
              <div className="flex flex-col w-full">
                ...
                <div className="md:flex w-full items-center mt-2 hidden">
                  ...
                  <div className="flex p-1 ps-3 pe-3 rounded-full bg-[#1a1b18] me-2 cursor-pointer hover:bg-[#383c34]" onClick={(e) => e.preventDefault()}>
                    <ChatCenteredText size={16} className="me-1"/>
                    <span className="text-xs">{postdetails.commentcount}</span>
                  </div>
                  <div className="flex p-1 ps-3 pe-3 rounded-full bg-[#1a1b18] cursor-pointer hover:bg-[#383c34]" onClick={(e) => e.preventDefault()}>
                    <Export size={16} className="me-1"/>
                    <span className="text-xs">Share</span>
                  </div>
                  <div className="flex ms-1 p-1 ps-3 pe-3 rounded-full bg-[#1a1b18] cursor-pointer hover:bg-[#383c34]" onClick={(e) => e.preventDefault()}>
