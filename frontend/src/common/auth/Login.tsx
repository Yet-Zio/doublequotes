  ...
  return (
    <div className="flex min-w-screen min-h-screen justify-center items-center">
      ...
      <form onSubmit={handleSubmit} className="absolute flex flex-col w-[530px] h-[644px] bg-[#1a1e15] rounded-2xl useinter p-10">
          ...
          <input value={loginDetails.id} onChange={(e) => {setLoginDetails({...loginDetails, id: e.target.value})}} type="text" className="mt-5 ms-5 me-5 p-4 ps-5 pe-5 rounded-3xl outline-0 bg-[#3e4535]/25 text-gray-400 placeholder-gray-400 hover:bg-[#3e4535]/50 focus:bg-[#3e4535]/50" placeholder="Email or username *" required/>
          ...
          <Link to={"/forgotpass"} className="mt-5 ms-5 text-sm text-[#A9A74F] w-[120px]">Forgot Password?</Link>
          <span className="mt-5 ms-5 text-sm">New to DoubleQuotes?&nbsp;<span className="text-[#A9A74F] cursor-pointer">Sign Up</span></span>
          <div className="flex flex-col h-40 justify-end">
            <button disabled={loginDetails.id === "" && loginDetails.password === ""} className={`mt-2 p-3 ps-7 pe-7 rounded-full font-semibold ${loginDetails.id !== "" && loginDetails.password !== "" ? "bg-[#A9A74F] hover:bg-[#A9A74F]/75" : "bg-[#3e4535]/10 text-[#3e4535]/45"}`}>Login</button>
          </div>
      </form>
    </div>
  );
